...
# dataset_id in database
twitter_id = 7

# how many objects to push to server at a time
batch_size = 1000

# where all topic folders are held
queries_folder = './queries_for_amita/'

# names of topic_folders: topic_id in database
topic_folders = {
    'abortion/':        3,
    'atheism/':         28,
    'climate_change/':  29,
...
def getFilesFromFolder(folder,topic_folder):
    all_files = []
    for file in os.listdir(folder+topic_folder):
...
def createTableObjects(jObj,session, topic_id):
    if jObj['id_str'] not in tweet_list:
        ...
        addTweetToSession(jObj,session, topic_id)
...
def addTweetToSession(jObj, session, topic_id):
...
def main(user=sys.argv[1],pword=sys.argv[2],db=sys.argv[3]):
    ...
    for topic_folder in topic_folders:
        topic_id = topic_folders[topic_folder]
        all_files = getFilesFromFolder(queries_folder, topic_folder)
        for dataFile in all_files:    
            print('Loading data from',dataFile,'for topic',topic_folder)
            with open(queries_folder+topic_folder+dataFile,'r', encoding='utf-8') as data:
                jObjs = []
                comment_index = 1
                for line in data:
                    ...
                    if len(jObjs) >= batch_size:
                        for jObj in jObjs:
                            ...
                            createTableObjects(jObj, session, topic_id)
                ...
                for jObj in jObjs:
                    createTableObjects(jObj,session,topic_id)
                session.commit()
                print("Pushed",comment_index,"comments total")
